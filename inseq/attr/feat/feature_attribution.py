...
class FeatureAttribution(Registry):
    ...
    def _detokenize(self, seq):
        tok = str
        if getattr(self.attribution_model, "detokenizer", None):
            tok = self.attribution_model.detokenizer.convert_tokens_to_string
        return [tok(x.token) for x in seq]
    ...
    def attribute(
        ...
        for attributions in out.sequence_attributions:
            attributions.source_txt = self._detokenize(attributions.source)
            attributions.target_txt = self._detokenize(attributions.target)
