...
class FeatureAttributionSequenceOutput(TensorWrapper, AggregableMixin):
    ...
    Attributes:
        source (list of :class:`~aminseq.utils.typing.TokenWithId`): Tokenized source sequence.
        target (list of :class:`~aminseq.utils.typing.TokenWithId`): Tokenized target sequence.
    ...
    def from_step_attributions(
        ...
        """Converts a list of :class:`~am_inseq.data.attribution.FeatureAttributionStepOutput` objects containing multiple
        examples outputs per step into a list of :class:`~am_inseq.data.attribution.FeatureAttributionSequenceOutput` with
        ...
        Returns:
            ...
            :class:`~am_inseq.data.attribution.FeatureAttributionSequenceOutput` objects.
        """
        attr = attributions[0]
    ...
    def show(
        ...
        **kwargs,
        ...
        """
        from am_inseq import show_attributions
...
class FeatureAttributionOutput:
    ...
    Attributes:
        sequence_attributions (list of :class:`~aminseq.data.FeatureAttributionSequenceOutput`): List
        ...
        step_attributions (list of :class:`~am_inseq.data.FeatureAttributionStepOutput`, optional): List
    ...
    def load(
        ...
        """Load saved attribution output into a new :class:`~am_inseq.data.FeatureAttributionOutput` object.

        Args:
            path (:obj:`str`): Path to the JSON file containing the saved attribution output.
                Note that the file must have been saved with the :meth:`~am_inseq.data.FeatureAttributionOutput.save`
        ...
        Returns:
            :class:`~aminseq.data.FeatureAttributionOutput`: Loaded attribution output
        """
    ...
    def aggregate(
        ...
        **kwargs,
        ...
        Returns:
            :class:`~aminseq.data.FeatureAttributionOutput`: Aggregated attribution output
        """
    ...
    def get_attributions(self, tokenizer):
        ParsedAttribution = namedtuple("ParsedAttribution",
                                       ("col_labels",
                                        ...
                                        "input_text",
                                        "generated_text"))
        _attributions = self.sequence_attributions[0].aggregate(None).to_dict()
        row_labels = [tokenizer.convert_tokens_to_string(tokens=x.token) for x in _attributions['target']]
        col_labels = row_labels[len(_attributions['source']):]
        np_arr = np.array(_attributions['target_attributions'], dtype=float)
        input_text = self.info['input_texts'][0]
        generated_text = self.info['generated_texts'][0][len(input_text):].strip()

        return ParsedAttribution(col_labels, row_labels, np_arr, input_text, generated_text)

    def get_dataframe(self, tokenizer):
        col_labels, row_labels, arr, input_text, generated_text = self.get_attributions(tokenizer)
        df = pd.DataFrame(arr)
        df.loc[-1] = col_labels
        df.index = df.index + 1
        df = df.sort_index()
    ...
    def merge_attributions(cls, attributions: List["FeatureAttributionOutput"]) -> "FeatureAttributionOutput":
        """Merges multiple :class:`~am_inseq.data.FeatureAttributionOutput` objects into a single one.
