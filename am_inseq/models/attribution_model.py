...
from ..utils import MissingAttributionMethodError, check_device, format_input_texts, get_default_device
...
class AttributionModel(ABC, torch.nn.Module):
    ...
    Attributes:
        ...
        attribution_method (:class:`~aminseq.attr.FeatureAttribution`): The attribution method used alongside the model.
    ...
    def set_attributed_fn(self, fn: str):
        if fn not in STEP_SCORES_MAP:
            raise ValueError(f"Unknown function: {fn}. Register custom functions with am_inseq.register_step_function")
    ...
    def get_attributed_fn(
        ...
        if isinstance(attributed_fn, str):
            if attributed_fn not in STEP_SCORES_MAP:
                raise ValueError(
                    f"Unknown function: {attributed_fn}. Register custom functions with am_inseq.register_step_function"
                )
        ...
        **kwargs,
        ...
        Args:
            ...
            output_step_attributions (:obj:`bool`, `optional`): Whether to fill the ``step_attributions`` field in
                :class:`~am_inseq.FeatureAttributionOutput` with step-wise attributions for each generated token. default:
                False.
            ...
            step_scores (:obj:`list(str)`): A list of step function identifiers specifying the step scores to be
                ...
                :func:`~am_inseq.list_step_functions`.
        ...
        Returns:
            :class:`~aminseq.FeatureAttributionOutput`: The attribution output object containing the attribution scores,
